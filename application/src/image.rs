...
impl<'i, Pixel> ImageViewMut<'i, Pixel> {
  ...
  pub fn window(&self, left_top: ImageSize, right_bottom: ImageSize) -> ImageView<'i, Pixel> {
  ...
  pub fn window_mut(&self, left_top: ImageSize, right_bottom: ImageSize) -> ImageViewMut<'i, Pixel> {
    window!(self, left_top, right_bottom, ImageViewMut);
  }

  pub fn as_view(&self) -> ImageView<'i, Pixel> {
  ...
  pub fn draw<SrcPixel>(
    ...
    if position.0 >= self.size.0 as isize || position.1 >= self.size.1 as isize
    || position.0 + src.size.0 as isize <= 0 || position.1 + src.size.1 as isize <= 0 {
        return;
    }
    ...
    let right_bottom_x = if position.0 + src.size.0 as isize > self.size.0 as isize {
      (self.size.0, (self.size.0 as isize - position.0) as usize)
    } else {
      ((position.0 + src.size.0 as isize) as usize, src.size.0)
    };

    let right_bottom_y = if position.1 + src.size.1 as isize > self.size.1 as isize {
      (self.size.1, (self.size.1 as isize - position.1) as usize)
    } else {
      ((position.1 + src.size.1 as isize) as usize, src.size.1)
    };
